...
const VIZ_OPTIONS = [
  ...
  { key: "line", label: "Line Chart", icon: LineChartIcon, description: "Trends over time" },
  { key: "pie", label: "Pie Chart", icon: PieChartIcon, description: "Proportions" },
  { key: "kpi", label: "KPI Card", icon: Gauge, description: "Single metric" },
  { key: "grid", label: "Metric Grid", icon: LayoutGrid, description: "Multiple metrics" },
  { key: "summary", label: "Summary", icon: FileText, description: "Text analysis" },
] as const;
    ...
    <div className="flex flex-col h-full overflow-hidden">
        ...
        <div className="px-4 pt-3 pb-1 max-w-4xl mx-auto w-full shrink-0">
          <div className="rounded-lg border bg-card p-3 space-y-2">
            <div className="flex items-center justify-between">
              <p className="text-sm font-medium">How would you like to see this data?</p>
              <Button variant="ghost" size="sm" className="text-xs h-7" onClick={handleSkipViz}>
                Let AI decide
              </Button>
            </div>
            <div className="grid grid-cols-3 sm:grid-cols-4 md:grid-cols-7 gap-1.5">
              {VIZ_OPTIONS.map((viz) => (
                ...
                >
                  ...
                  <span className="text-xs font-medium leading-tight">{viz.label}</span>
                </button>
              ))}
            </div>
          </div>
      ...
      <div className="p-4 max-w-4xl mx-auto w-full shrink-0">
        ...
        <p className="text-xs text-muted-foreground mt-2 text-center">
          IntentBoard generates safe, read-only queries on your authorized data.
        </p>
      </div>
    </div>
  );
  ...
  return (
    <div className="flex flex-col items-center justify-center py-16 space-y-6">
      ...
      <div className="text-center space-y-2">
        <h3 className="text-lg font-semibold">Welcome to IntentBoard</h3>
