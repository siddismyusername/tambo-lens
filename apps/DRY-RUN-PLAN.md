# Tambo Lens — Module-by-Module Dry-Run Testing Plan
...
try {
  createDataSourceSchema.parse({ type: "postgresql" });
} catch (e) {
  /* should throw ZodError with .issues */
}
...
const none = await queryOne("SELECT * FROM data_sources WHERE id = $1", [
...
console.assert(
  ...
  "Should omit encrypted password",
);
